...
class ReverseTCPPayloadMixin(with_metaclass(ExploitOptionsAggregator, object)):
    ...
    lhost = THGOptIP('', 'Connect-back IP address')
    lport = THGOptPort(5555, 'Connect-back TCP Port')


class BindTCPPayloadMixin(with_metaclass(ExploitOptionsAggregator, object)):
    handler = PayloadHandlers.BIND_TCP
    rport = THGOptPort(5555, 'Bind Port')


class BasePayload(BaseExploit):
    architecture = None
    handler = None
    encoder = THGOptString("", "Encoder")
    fmt = None
    ...
    def get_encoders(self):
        path = "thgconsole/modules/encoders/{}".format(self.architecture)
    ...
    def get_encoder(self, encoder):
        module_path = "thgconsole/modules/encoders/{}".format(encoder).replace("/", ".")
...
class ArchitectureSpecificPayload(BasePayload):
    output = THGOptString("python", "Output type: elf/c/python")
    filepath = THGOptString("/tmp/{}".format(random_text(8)), "Output file to write")
