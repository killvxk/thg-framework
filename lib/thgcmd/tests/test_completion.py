...
# flake8: noqa E302
"""
Unit/functional testing for readline tab-completion functions in the thgcmd.py module.

These are primarily tests related to readline completer functions which handle tab-completion of thgcmd/cmd commands,
...
class CompletionsExample(cmd2.Cmd):
    """
    Example thgcmd application used to exercise tab-completion tests
    """
    def __init__(self):
...
class SubcommandsWithUnknownExample(cmd2.Cmd):
    """
    Example thgcmd application where we a base command which has a couple sub-commands
