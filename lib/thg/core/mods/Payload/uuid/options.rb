...
class BaseAuxiliary_iax2:
    ...
    def register_llmnr(self, timeout_value=5, threads_value=1):
      ...
      self.register_options([
        BaseOption(name='PayloadUUIDSeed', required=False, description='A string to use when generating the payload UUID (deterministic)'),
        BaseOption(name='PayloadUUIDRaw', required=False, description='A hex string representing the raw 8-byte PUID value for the UUID'),
        BaseOption(name='PayloadUUIDName', required=False, description='A human-friendly name to reference this unique payload (requires tracking)'),
        BaseOption(name='PayloadUUIDTracking', required=True, description='Whether or not to automatically register generated UUIDs',value=False),
