...
class BaseAuxiliary:
  ...
  def register_crawler(self, timeout_value=5, threads_value=1):
    ...
    self.register_options([
      ...
      BaseOption(name="RPORT",required=True,description="port",volue=80),
      ...
      BaseOption(name='DirBust', required=False, description='Bruteforce common URL paths', volue=True),
      BaseOption(name='RequestTimeout', required=False, description='The maximum number of seconds to wait for a reply', volue=15),
      BaseOption(name='RedirectLimit', required=False, description='The maximum number of redirects for a single request', volue=5),
      BaseOption(name='RetryLimit', required=False, description='The maximum number of attempts for a single request', volue=5),
      BaseOption(name='UserAgent', required=True, description='The User-Agent header to use for all requests',volue="Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)"),
