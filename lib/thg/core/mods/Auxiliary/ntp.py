...
class BaseAuxiliary_Ntp:
  ...
  def register_Ntp(self, timeout_value=5, threads_value=1):
    ...
    self.register_options([
        BaseOption(name='RPORT', required=False, description="RPORT",value=123),
        BaseOption(name='ENTRIES', required=False, description="PII Entry Count", value=1000),
        BaseOption(name='VERSION', required=True, description='Use this NTP version', value=2),
        BaseOption(name='IMPLEMENTATION', required=True, description='Use this NTP mode 7 implementation', value=3)
    ])
