...
class BaseAuxiliary_Redis:
    ...
    def register_redis(self, timeout_value=5, threads_value=1):
      ...
      self.register_options([
            ...
            BaseOption(name='RPORT',required=True, description='port',value=6379),
            BaseOption(name='PASSWORD',required=False,description='Redis password for authentication test', value='foobared'),
            BaseOption(name='READ_TIMEOUT',required=True,description='Seconds to wait while reading redis responses', value=2)
      ])
