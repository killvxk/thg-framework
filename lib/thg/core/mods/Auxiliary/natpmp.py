...
class BaseAuxiliary_Natpmp:
  ...
  def register_Natpmp(self):
    self.target_type = "http"
    self.register_options([
          BaseOption(name='RPORT', required=True, description="list of hosts",value=""),
          #Opt::CHOST,
          BaseOption(name='LIFETIME', required=True, description="Time in ms to keep this port forwarded (set to 0 to destroy a mapping)", value=3600000),
          BaseOption(name='PROTOCOL',  required=True, description="Protocol to forward",value='TCP')
