...
class BaseAuxiliary_Rservices:
    ...
    def register_Rservices(self, timeout_value=5, threads_value=1):
      ...
      self.register_options([
        ...
        BaseOption(name='FROMUSER', required=True, description='The username to login from'),
        BaseOption(name='FROMUSER_FILE', required=True, description='File containing from usernames, one per line'),
        BaseOption(name='REMOVE_FROMUSER_FILE', required=True, description="Automatically delete the FROMUSER_FILE on module completion", value=False)
      ])
