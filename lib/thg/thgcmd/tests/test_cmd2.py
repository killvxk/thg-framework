...
def test_base_py(base_app):
    ...
    assert 'thgcmd.thgcmd.Cmd object' in out[0]
...
def test_base_debug(base_app):
    ...
    # Make sure we get an exception, but thgcmd handles it
...
def test_base_cmdloop_with_queue(outsim_app):
    # Create a thgcmd.Cmd() instance and make sure basic settings are like we want for test
    outsim_app.use_rawinput = True
    intro = 'Hello World, this is an intro ...'
    outsim_app.cmdqueue.append('quit\n')

    # Need to patch sys.argv so thgcmd doesn't think it was called with arguments equal to the py.test args
    testargs = ["prog"]
    expected = intro + '\n'
...
def test_clipboard_failure(base_app, capsys):
    # Force thgcmd clipboard to be disabled
...
def test_raw_input(base_app):
    ...
    line = base_app.pseudo_raw_input('(thgcmd)')
...
def test_stdin_input():
    ...
    line = app.pseudo_raw_input('(thgcmd)')
    assert line == fake_input
...
class ReplWithExitCode(cmd2.Cmd):
    """ Example thgcmd application where we can specify an exit code when existing."""
